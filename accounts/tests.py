...
class AccountBooksTestCase(TestCase):
    ...
    def test_update_summary(self):
        ...
        order2 = Order.objects.create(
            account=self.account, status="ok", total_price_real=Decimal("50.00")
        )
        ...
        self.assertEqual(
            books.money_total, Decimal("300.00")
        )  # order1 + order2 + order3 (应收总和)
...
class ApiValidationTestCase(TestCase):
    def setUp(self):
        ...
        self.goods = GoodsInfo.objects.create(
            goods="Banana", goods_price=Decimal("5.00")
        )
