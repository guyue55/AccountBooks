    ...
    def post(self, request, *args, **kwargs):
        ...
        if user is not None:
            ...
            return render(
                request,
                self.template_name,
                {"error": "用户名或密码错误，请重试。", "username": username},
            )
...
class RegisterView(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if password != password_confirm:
            return render(
                request, self.template_name, {"error": "两次输入的密码不一致。"}
            )
...
class AjaxFormMixin:
    ...
    def get(self, request, *args, **kwargs):
        ...
        if self._is_ajax(request) and self.partial_template_name:
            ...
            self.object = (
                self.get_object()
                if hasattr(self, "get_object") and "pk" in kwargs
                else None
            )
            form = self.get_form()
...
class DashboardView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        collection_rate = (
            round(total_over / total_revenue * 100, 1) if total_revenue > 0 else 0
        )
        ...
        debt_books = AccountBooks.objects.filter(money_wait__gt=0).order_by(
            "-money_wait"
        )
        debt_distribution = []
        if debt_books.count() > 5:
            ...
            for b in top_5:
                debt_distribution.append(
                    {"name": b.account_info.name, "value": float(b.money_wait)}
                )
            others_sum = others.aggregate(total=Sum("money_wait"))["total"] or 0
            if others_sum > 0:
                debt_distribution.append(
                    {"name": "其他客户", "value": float(others_sum)}
                )
        else:
            for b in debt_books:
                debt_distribution.append(
...
class OrdersView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if q:
            qs = qs.filter(
                Q(account__name__icontains=q) | Q(items__goods__goods__icontains=q)
            ).distinct()
    ...
    def get(self, request, *args, **kwargs):
        ...
        if self._is_ajax(request):
            ...
            context = self.get_context_data(
                form=form, formset=formset, title="新增交易记录"
            )
            return render(request, self.partial_template_name, context)
...
class GoodsDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if request.headers.get("X-Requested-With") == "XMLHttpRequest":
                return JsonResponse(
                    {"success": False, "message": f"删除失败: {str(e)}"}, status=500
                )
        return redirect(self.success_url)
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not ids or not status:
                return JsonResponse(
                    {"success": False, "message": "参数错误"}, status=400
                )
            ...
            logger.error(
                f"User {request.user} batch update Orders status failed: {str(e)}"
            )
            return JsonResponse(
                {"success": False, "message": f"批量更新失败: {str(e)}"}, status=500
            )
...
class CustomerBatchDeleteView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.info(
                f"User {request.user} batch soft-deleted {deleted_count} Customers"
            )
            return JsonResponse(
                {"success": True, "message": f"成功删除 {deleted_count} 名顾客"}
            )
        except Exception as e:
...
class ExportOrdersView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for order in orders:
            items_desc = " | ".join(
                [f"{item.goods.goods}x{item.quantity}" for item in order.items.all()]
            )
            writer.writerow(
                [
                    order.id,
...
class ExportAccountBooksView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        books = AccountBooks.objects.select_related("account_info").order_by(
            "-money_wait"
        )
        for book in books:
            writer.writerow(
