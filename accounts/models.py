...
class UserProfile(models.Model):
    ...
    theme = models.CharField(
        max_length=20, choices=THEME_CHOICES, default="dark", verbose_name="系统主题"
    )
...
class AccountInfo(SoftDeleteModel):
    ...
    real_name = models.CharField(
        max_length=50, verbose_name="真实姓名", default="", blank=True
    )
    age = models.IntegerField(verbose_name="年龄", default=0, blank=True)
    phone = models.CharField(
        max_length=20, verbose_name="联系电话", default="", blank=True
    )
    location = models.CharField(
        max_length=100, verbose_name="地址", default="未知地区", blank=True
    )
    remarks = models.CharField(
        max_length=200, verbose_name="备注", default="", blank=True
    )
...
class Order(SoftDeleteModel):
    ...
    def __str__(self):
        """返回订单详情摘要。."""
        return (
            f"订单ID:{self.id} | 顾客:{self.account.name} | "
...
class OrderItem(SoftDeleteModel):
    ...
    goods = models.ForeignKey(GoodsInfo, on_delete=models.PROTECT, verbose_name="商品")
...
def update_account_books(sender, instance, **kwargs):
    ...
    account_book, _ = AccountBooks.objects.get_or_create(account_info=instance.account)
